...
class AKIAgentTest(unittest.TestCase):
    def test_normalize_prediction_calls(self):
        from src.agents.aki import AKIPrediction, normalize_prediction_calls

        prediction = AKIPrediction(
            sample_id="sample_1",
            person_id=1,
            agent_id="nephrologist",
            aki_any_probability=0.2,
            aki_3mo_probability=0.5,
            aki_6mo_probability=0.8,
            aki_any_call="yes",
            aki_3mo_call="no",
            aki_6mo_call="uncertain",
            confidence=0.7,
            rationale="Risk factors are mixed.",
        )

        normalized = normalize_prediction_calls(prediction)

        self.assertEqual(normalized.aki_any_call, "no")
        self.assertEqual(normalized.aki_3mo_call, "uncertain")
        self.assertEqual(normalized.aki_6mo_call, "yes")
